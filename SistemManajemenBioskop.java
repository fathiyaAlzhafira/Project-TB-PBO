...
public class SistemManajemenBioskop {
    ...
    try (Connection connection = DatabaseConnection.getConnection();
        ...
        if (resultSet.next()) {
            String judul = resultSet.getString("judul");
            double hargaTiket = resultSet.getDouble("harga_tiket");
            double totalHarga = hargaTiket * jumlahTiket; // Perhitungan matematika
            ...
            if (konfirmasi == 'Y' || konfirmasi == 'y') { // Percabangan untuk konfirmasi
                ...
                try (PreparedStatement statementTiket = connection.prepareStatement(sqlTiket)) {
                    ...
                    statementTiket.setInt(3, jumlahTiket);
    ...
    System.out.println("========================================");
    String sql = "SELECT f.judul, SUM(t.jumlah) AS tiket_terjual, " +
    ...
    try (Connection connection = DatabaseConnection.getConnection();
        ...
        while (resultSet.next()) {
            ...
            int tiketTerjual = resultSet.getInt("tiket_terjual");
            double pendapatan = resultSet.getDouble("pendapatan");
            totalPendapatan += pendapatan;

            System.out.printf("%-16s | %-13d | Rp %.2f%n", judulFilm, tiketTerjual, pendapatan);
        }
